...
export default function withActiveLinkDetection(WrappedComponent) {
  return class WithActiveLinkDetection extends Component {
    static displayName = `WithActiveLinkDetection(${getDisplayName(WrappedComponent)})`;
    ...
    componentDidMount() {
      ...
      if (this.moreDropdownElement) {
        ...
        window.addEventListener('NavItemsResized', this.checkActiveLink, { passive: true });
      }
    }

    componentWillUnmount() {
      this.moreDropdownElement &&
        window.removeEventListener('NavItemsResized', this.checkActiveLink, { passive: true });
    }
    ...
    checkActiveLink() {
      ...
      window.requestAnimationFrame(() => {
        ...
        const selectedLink =
          ...
          this.moreDropdownElement.querySelector('[data-ux="NavMoreMenuLinkActive"]') ||
          this.moreDropdownElement.querySelector('[data-ux="NavMoreMenuLinkNestedActive"]');
