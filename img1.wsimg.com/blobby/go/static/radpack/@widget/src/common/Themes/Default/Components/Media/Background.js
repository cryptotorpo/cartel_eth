...
import { resolveImageSource, addImageApiArgs, isInternalUrl } from '../../../../utils/image';
...
function buildMediaQueries(
  backgroundImage,
  { generateGradient, desktopWidth, mobileWidth, blur, disableOverlayMobile, isPublish }
) {
  return ranges.reduce((acc, curr) => {
    ...
    (blur ? [1] : densities).forEach(density => {
      ...
      const gradientCSS =
        generateGradient &&
        generateGradient(curr.isMobile || curr.isTablet ? !disableOverlayMobile : true);
...
export function Background({
  ...
  const { widgetType, widgetPreset, context, category, renderMode, order } = this.base;
  const { parallaxEnabled, position, alt } = imageData;
  ...
  const mutatorConfiguration = this.constructor.getMutatorDefaultProps(widgetType, widgetPreset);
  ...
  const _bgStyle = {
    backgroundPosition: parallaxEnabled ? 'center' : position || backgroundPosition || 'center',
    ...
    if (overlayAlpha >= 0) {
      if (widgetType === 'HEADER') {
        ...
        opacity = hasOpacitySlider
          ? overlayAlpha
          : this.constructor.getDefaultImageOverlayOpacity(widgetType, widgetPreset);
      } else {
        opacity = overlayAlpha;
      }
    } else {
      opacity = this.constructor.getDefaultImageOverlayOpacity(widgetType, widgetPreset);
    }
  ...
  if (resolvedSrc2) {
    _bgStyle.backgroundImage = `${resolvedSrc}, url("${resolvedSrc2.replace(/"/g, '\\"')}")`;
    if (backgroundSize2) {
  ...
  const shouldLazyLoad =
    lazyLoad && isPublish && !isLazyLoadedWidget && !(widgetType === HEADER || order === 0);
