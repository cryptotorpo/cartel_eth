  ...
  return this.merge(
    {
      ...
      children: [
        <Input
          ...
          { ...props }
        />,
        <Label key='label' group='InputFloatLabel' for={ inputId } children={ placeholder } />
      ],
      style: {
        position: 'relative'
      }
...
export function ButtonPrevious(props) {
  return this.merge(
    {
      tag: 'button',
      children: <UX2.Element.Icon icon='chevronLeft' data-edit-interactive={ true } />
    },
    props
  );
}
...
export function ButtonNext(props) {
  return this.merge(
    {
      tag: 'button',
      children: <UX2.Element.Icon icon='chevronRight' data-edit-interactive={ true } />
    },
    props
  );
}
...
export function SlideshowArrowsButtonPrevious(props) {
  return this.ButtonPrevious(this.merge(commonArrowProps, slideshowButtonProps, props));
}

export function SlideshowArrowsButtonNext(props) {
  return this.ButtonNext(this.merge(commonArrowProps, slideshowButtonProps, props));
}
...
export function LinkIcon({
  ...
  const modifiedChildren = children
    ? [<UX2.Element.Element key='span' style={{ verticalAlign: 'middle' }} children={ children } />]
    : [];
  const iconElement = (
    <UX2.Element.Icon
      data-aid={ icon }
  ...
  right ? modifiedChildren.push(iconElement) : modifiedChildren.unshift(iconElement);
...
export function MoreLinkExpand({ expanded, ...props }) {
  ...
  return this.MoreLink(this.merge({ icon, ['aria-expanded']: expanded }, props));
}
