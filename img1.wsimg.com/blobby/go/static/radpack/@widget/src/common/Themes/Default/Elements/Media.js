...
import { resolveImageSource, getImageDimensionData } from '../../../utils/image';
...
export function Icon({
  ...
  if (typeof size === 'object' && 'width' in size && 'height' in size) {
    iconSizeWidth = size.width;
    iconSizeHeight = size.height;
  } else {
    iconSize = this.mapPropValue('iconSize', size);
  }
  ...
  return this.merge(
    {
      ...
      children: svgPath,
      viewBox:
        iconSizeWidth && iconSizeHeight ? `0 0 ${iconSizeWidth} ${iconSizeHeight}` : '0 0 24 24',
      fill: 'currentColor',
      width: iconSizeWidth || iconSize,
      height: iconSizeHeight || iconSize,
      style: {
        color: 'inherit',
...
export function IconAiro(props) {
  const color = this.base.category === 'neutral';
...
export function Image({
  ...
  const reserveMobileHeight =
    renderMode === PUBLISH && !isLazyLoadedWidget && aspectRatio && isFullWidthMobile;
  ...
  const mergedImageConfig = this.merge(
    {
      style: {
        ...
        '@xs-only': reserveMobileHeight
          ? { height: `calc((100vw - ${mobileGutterWidth * 2}px)/${aspectRatio})` }
          : {},
        'aspectRatio': aspectRatio ? `${aspectRatio} / 1` : 'auto'
      },
      tag: 'img',
      alt: alt || get(parsedImageData, 'alt') || title || src,
      lazyLoad: lazyLoad && renderMode === PUBLISH && !isLazyLoadedWidget && !eagerLoadImages,
      mobileGutterWidth,
      reserveMobileHeight
    },
    props,
